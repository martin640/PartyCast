...
public class LocalLibraryProvider implements LibraryProvider, JSONable {
    ...
    int libraryItemIdPool;
    Map<Integer, LocalLibraryItem> localLibraryItemMap;
    ...
    @Override
    public int pushIdFromPool() {
        return ++libraryItemIdPool;
    }

    void registerLibItem(LocalLibraryItem item) {
        item.id = pushIdFromPool();
